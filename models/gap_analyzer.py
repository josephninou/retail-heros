...
class GapAnalyzer:
    def __init__(self):
        self.gap_threshold = 30  # pixels
        self.min_gap_area = 500  # pixels²
    
    def detect_gaps(self, image, detections):
        """
        Détecte les espaces vides / ruptures de stock
        """
        try:
            if not detections:
                return []
            ...
            # Créer un masque des zones occupées
            ...
            # Dilater pour identifier les zones autour des produits
            kernel = np.ones((20, 20), np.uint8)
            dilated_mask = cv2.dilate(occupied_mask, kernel, iterations=1)
            
            # Trouver les espaces vides
            gap_mask = cv2.bitwise_not(dilated_mask)
            
            # Trouver les contours des gaps
    ...
    def estimate_missing_product(self, x, y, w, h, detections):
        """
        Estime quel produit manque à un emplacement donné
        """
        # Analyser les produits voisins
        nearby_products = []
        for det in detections:
            ...
            if distance < 200:  # Seuil de proximité
    ...
    def visualize_gaps(self, image, gaps):
        """
        Visualise les gaps sur l'image
        """
        img_copy = image.copy()
        for gap in gaps:
            x, y, w, h = gap['position']
            cv2.rectangle(img_copy, (x, y), (x+w, y+h), (0, 0, 255), 3)
            cv2.putText(img_copy, "RUPTURE", (x, y-10), 
